...
def _calculate_full_minutes_range(startSession, endSession, calendar):
    schedule = calendar.schedule[(calendar.schedule.index <= endSession) & (calendar.schedule.index >= startSession)]
    NANOS_IN_MINUTE = 60000000000
    opens_in_ns = schedule['market_open'].values.astype('datetime64[ns]')
    closes_in_ns = schedule['market_close'].values.astype('datetime64[ns]')
    ...
    for day_idx, size in enumerate(daily_sizes):
        ...
        size_int = int(size)
        all_minutes[idx:(idx + size_int)] = np.arange(
            opens_in_ns[day_idx],
    ...
    return pd.DatetimeIndex(all_minutes).tz_localize("UTC")
    ...
    def __init__(self,
        ...
        if len(sessions) == 0:
            ...
            if self.emission_rate == "minute":
                ...
                minutes_back = trading_calendar.minutes_for_sessions_in_range(
                    sessions[0],
                    sessions[-1]
                )
                minutes = _calculate_full_minutes_range(sessions[0], sessions[-1], trading_calendar)
